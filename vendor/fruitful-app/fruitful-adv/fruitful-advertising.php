	...
	if (!class_exists('FruitfulAdv')) {
		...
		abstract class FruitfulAdv
		{
			...
			public function get_advertising()
			{
				...
				return (!is_wp_error($response)) ? array(
					'product_name' => $params['product_name'],
					'advertising' => (array)json_decode($response['body'])
				) : false;
			}
			...
			public function init_advertising_option()
			{
				...
				set_transient('ffc_advertising_option_actual_' . $product_info['product_name'], true,
					24 * HOUR_IN_SECONDS);
			}
		}
	}
