...
class Fruitful_Widget_News_Archive extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('News-Archive') : $instance['title'], $instance, $this->id_base);
...
class Fruitful_Widget_Last_Tweets extends WP_Widget {
	
	function Fruitful_Widget_Last_Tweets () {
		$widget_ops = array( 'classname' => 'widget_last_tweets', 'description' => __( 'Use this widget to show your last tweets', 'fruitful' ) );
		$this->WP_Widget( 'widget_last_tweets', __( 'Fruitful Last Tweets', 'fruitful' ), $widget_ops );
		$this->alt_option_name = 'widget_last_tweets';
	}
	...
	function widget( $args, $instance ) {
		...
		<?php 
			...
			$transientName = 'list-tweets'; 
			$cacheTime = 5;
			
			if( ( false === ($tweets = get_transient($transientName)) )  || (count($tweets) != $number) || ($tweets[0]['name'] != $username) ) :
				unset($tweets);
				
				$json = wp_remote_get("http://api.twitter.com/1/statuses/user_timeline.json?screen_name=$username&count=$number");
				$dataTweets = json_decode($json['body'], true);

				if ( !$dataTweets['errors'] && $username ) {
						
					foreach ($dataTweets as $tweet) :
						
						$user = $tweet['user']['name'];
						$user_link = 'http://twitter.com/#!/'. $user;
						$link = 'http://twitter.com/#!/'. $user .'/status/'. $tweet['id_str'];
						...
						$createdAt = date_parse($createdAt);
						$uTime = mktime($createdAt['hour'], $createdAt ['minute'], $createdAt ['second'], $createdAt ['month'], $createdAt ['day'], $createdAt ['year']);
					...
					set_transient($transientName, $tweets, 60 * $cacheTime);
				} else {
					echo 'Wrong twitter username';
				}	
			endif; 
		...
		if ( $tweets ): 	?>
			...
			<div class="tweets_list">
				<p class="tweets_user_title"><a href="<?php echo $tweets[0]['user_link']; ?>">
					<img src="<?php echo $tweets[0]['image']; ?>" width="48" height="48" alt="" />
					<?php echo $tweets[0]['name'];?>
				</a></p>
				<ul>
					<?php foreach($tweets as $t) : ?>						
						<li>
							<a href="<?php echo $t['link']; ?>" class="tweet-time"><?php echo human_time_diff($t['time'], current_time('timestamp')); ?> ago:</a>
							<?php echo $t['text']; ?>
						</li>
					<?php endforeach; ?>
				</ul>
		...
		wp_enqueue_style( 'wdgt_news_style', 	get_template_directory_uri() . '/inc/css/twitter.css');
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of tweets to show:', 'fruitful' ); ?></label>
