	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			self::$instance =& $this;
			 ...
			 $this->strings = array(
                ...
                'oops'                           => __( 'Something went wrong.', 'fruitful' ),
                'notice_can_install_required'    => __( 'Fruitful theme requires the following plugin: %1$s.', 'fruitful' ),
                'notice_can_install_recommended' => __( 'Fruitful theme recommends the following plugin: %1$s.', 'fruitful' ),
                'notice_cannot_install'          => __( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'fruitful'),
                'notice_can_activate_required'   => __( 'The following required plugin is currently inactive: %1$s.','fruitful' ),
                'notice_can_activate_recommended'=> __( 'The following recommended plugin is currently inactive: %1$s.', 'fruitful' ),
                'notice_cannot_activate'         => __( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'fruitful'),
                'notice_ask_to_update'           => __( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with Fruitful theme: %1$s.', 'fruitful'),
                'notice_cannot_update'           => __( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'fruitful'),
                'install_link'                   => __( 'Begin installing plugin', 'fruitful' ),
                'activate_link'                  => __( 'Begin activating plugin', 'fruitful' ),
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), 'fruitful' ); ?>
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$rendered .= '<p>' . sprintf( __( $this->strings[$type], $count, 'tgmpa' ), $imploded, $count ) . '</p>'; // All messages now stored
					}

					/** Setup variables to determine if action links are needed */
					$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '">' . __( $this->strings['install_link'], $install_link_count, 'tgmpa' ) . '</a>' : '';
					$show_activate_link = $activate_link ? '<a href="' . admin_url( 'plugins.php' ) . '">' . __( $this->strings['activate_link'], $activate_link_count, 'tgmpa' ) . '</a>'  : '';
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'fruitful' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( 'Private Repository', 'fruitful' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( 'Pre-Packaged', 'fruitful' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( 'WordPress Repository', 'fruitful' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'fruitful' ) : __( 'Recommended', 'fruitful' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'fruitful' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'fruitful' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'fruitful' ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'fruitful' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'fruitful' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'fruitful' ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'fruitful' );
				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'fruitful' );
				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'fruitful' );
				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'fruitful' );
				$this->strings['process_failed']      = __( 'Plugin install failed.', 'fruitful' );
				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'fruitful' );
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					...
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'fruitful' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'fruitful' ) . '</span><span class="hidden">' . __( 'Hide Details', 'fruitful' ) . '</span>.</a>';
				...
				/** Default installation strings */
					...
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'fruitful' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'fruitful' ) . '</span><span class="hidden">' . __( 'Hide Details', 'fruitful' ) . '</span>.</a>';
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'fruitful' ) . '">' . __( 'Return to the Dashboard', 'fruitful' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
