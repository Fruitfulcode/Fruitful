	...
	class fruitful_gallery
	{
		...
		public function save_postdata($post_id) {
			...
			if(isset($_POST['fruitful-gallery'])) {
				...
				$this->save_meta_data($post_id, $_POST['fruitful-animations-speed'], 	get_post_meta($post_id, 'fruitful-animations-speed', true), 'fruitful-animations-speed');
		...
		public function fruitful_add_new_element() {
			...
			$out .= '<li class="glr_item">' . "\r\n";
				...
				$out .= '<a href="javascript.void(0)" class="delete_btn_glr" title="Delete Images" title=""></a>' . "\r\n";
			$out .= '</li>';
		...
		public function custom_gallery_columns($columns) {
				$columns = array(
									...
									'title' 	=> __( 'Gallery', 'fruitful' ),
									"author" 	=> __( 'Author', 'fruitful'  ),
									'shortcode'	=> __( 'Shortcode', 'fruitful'  ),
									'date' 		=> __( 'Date' , 'fruitful' )
				);
			return $columns;
		}
		...
		public function manage_gallery_columns( $column, $post_id ) {
			global $post;
				switch( $column ) {
					case 'shortcode' :
							if ( empty( $shortcode ) )  echo '[grid-gallery id='.$post_id.']';
					break;
				default :
					break;
			}
